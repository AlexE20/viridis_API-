...
const deletePlantById = async (req, res) => {
    ...
    try {
        const plantDeleted = await plantService.deletePlantById(userId,gardenId,userPlantId);
        if (!plantDeleted) {
...
const addUserPlant = async (req, res) => {
    ...
    try {
        const userPlant = await plantService.addUserPlant(userId, plantId, gardenId);
        if (!userPlant) {
